...
module.exports = async (d) => {
    ...
    const [guildID = d.guild.id, variable, type = "asc", custom = `{top}. {username}: {value}`, list = 10, page = 1, table = d.client.mysql.tables[0], hideNegativeValue = "false", hideZeroValue = "false"] = data.inside.splits;
    ...
    if (!type || (type.toLowerCase() !== "asc" && type.toLowerCase() !== "desc"))
        return d.aoiError.fnError(d, "custom", {}, `Invalid type must be "desc" or "asc"`);
    const result = [];
    const guild = await d.util.getGuild(d, guildID);
    let db = await d.client.mysql.all(table, (data) => {
    ...
    for (const lbdata of db) {
        ...
        if (!member)
            continue;
        if (hideNegativeValue === "true" && parseInt(lbdata?.value) < 0)
            continue;
        if (hideZeroValue === "true" && parseInt(lbdata?.value) == 0)
            continue;
        ;
        const replacer = {
