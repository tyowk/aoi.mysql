...
module.exports = async (d) => {
    ...
    const [variable, order = "asc", type = "user", custom = "{top}. {name}: {value}", list = 10, page = 1, table = d.client.mysql.tables[0],] = data.inside.splits;
    if (!d.client.variableManager.has(variable?.addBrackets()))
        return d.aoiError.fnError(d, "custom", {}, `Variable ${variable.addBrackets()} Not Found!`);
    if (!order || (order.toLowerCase() !== "asc" && order.toLowerCase() !== "desc"))
        return d.aoiError.fnError(d, 'custom', {}, `Invalid order must be "desc" or "asc"`);
    let y = 0;
    let value;
    let content = [];
    let all = await d.client.mysql.findMany(table, (data) => data.key.startsWith(variable.deleteBrackets() + "_") && data.key.split("_").length === (type === "user" ? 3 : 2));
    ...
    for (let i = 0; i < all.length; i++) {
        ...
        if (user) {
            ...
            if (text.includes("{execute:")) {
                ...
                if (!awaited)
                    return d.aoiError.fnError(d, "custom", { inside: data.inside }, ` Invalid awaited command '${ins}' in`);
