    ...
    private async _connect() {
        try {
            ...
            this._logger([
                ...
                { text: `Installed on v${pkg?.version || '0.0.0'}`, textColor: 'green' }
            ], { text: ' aoi.mysql ', textColor: 'cyan' });

            const res = await (await fetch('https://registry.npmjs.org/aoi.mysql')).json();
            if (!res?.versions[pkg?.version] || res['dist-tags']?.dev === pkg?.version || pkg?.version?.includes('dev')) return this._logger([
                { text: 'This is a development version. Some stuff may be incomplete or unstable.', textColor: 'red' }
            ], { text: 'aoi.mysql', textColor: 'yellow' });

            if (pkg?.version !== res['dist-tags']?.latest) return this._logger([
                    { text: 'Outdated version detected! update with "npm i aoi.mysql@latest"', textColor: 'red' }
                ], { text: 'aoi.mysql', textColor: 'yellow' });;
        } catch (err) {
