...
module.exports = async (options, db) => {
  ...
  if (!fs.existsSync(backupDir)) {
    error(`The "${chalk.red(options.backup.directory)}" folder does not exist.`);
    return;
  }
  ...
  // Calculate total keys to transfer
  ...
  success(chalk.yellow("This process may take a while depending on the amount of data. Canceling this process will lose current progress."));
  ...
  for (const dir of directories) {
    ...
    if (fs.statSync(dirPath).isDirectory()) {
      ...
      for (const file of files) {
        ...
        await delay(1000); // Simulate preparation time
        ...
        await delay(2000); // Simulate transfer time
