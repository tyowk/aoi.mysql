module.exports = async (d: any) => {
    ...
    let [varname, userID = d.author?.id, guildID = d.guild?.id || 'dm', table = d.client.mysql.tables[0]] = data.inside.splits;
    ...
    data.result =
        (await d.client.mysql.get(table, varname, `${userID}_${guildID}`))?.value ??
