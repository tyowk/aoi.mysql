module.exports = async (d: any) => {
  ...
  const [variable, id, type = "guild", format, table = d.client.mysql.tables[0]] = data.inside.splits;
  ...
  let db = await d.client.mysql.findMany(table, (data: any) => {
    return data.key.startsWith(key.split("_")[0]) && (type === "guild" ? data.key.split("_")[2] === key.split("_")[2] : true)
  });

  switch (format) {
    case "top":
      data.result = db.sort((a: any, b: any) => b.value - a.value).findIndex((x: any) => x.key === key) + 1 || 0;
      break;
    case "value":
      data.result = db.find((x: any) => x.key === key)?.value || 0;
      break;
    case "id":
