module.exports = async (d: any) => {
    ...
    let [varname, userID = d.author?.id, table = d.client.mysql.tables[0]] = data.inside.splits;
    ...
    data.result = (await d.client.mysql.get(table, varname, userID))?.value ?? d.client.variableManager.get(varname, table)?.default;
