module.exports = async (d: any) => {
    ...
    let value;
    let content = [];
    let all = await d.client.mysql.findMany(table, (data: any) => data.key.startsWith(variable.deleteBrackets() + "_") && data.key.split("_").length === (type === "user" ? 3 : 2));
    all = all.filter((x: any, i: any, y: any) => y.findIndex((e: any) => e.key === x.key) === i);

    all = all.sort((x: any, y: any) => { return Number(y.value) - Number(x.value)});
  
    const getdata = async (user: any, Data: any, key: any) => {
        user =
            (type === "globalUser"
    ...
    for (let i = 0; i < all.length; i++) {
        ...
        if (d.client.mysql.type === "aoi.db") value = Number(Data?.value);
