const pluralize = (num: any, txt: any, suffix = "s") => `${num} ${txt}${num !== 1 ? suffix : ""}`;
...
export class Time {
    ...
    static parse(time: any): any {
        ...
        if (typeof time === "number")
            ...
            const ms = data.map((x: any) => x.ms).reduce((a: any, b: any) => a + b, 0);
            const format = data
                .filter((x: any) => x.format)
                .map((x: any) => x.format)
                .join(", ");
